#define pr_fmt(fmt)	"[Display][sw49109-ops:%s:%d] " fmt, __func__, __LINE__
...
static char sha_ctrl_values[NUM_SHA_CTRL] = {0x04, 0x0D, 0x1A, 0x30, 0xD2};
...
int lge_panel_power_on_sw49109(struct dsi_panel *panel)
{
	int rc = 0;
...
#if IS_ENABLED(CONFIG_LGE_DISPLAY_EXTERNAL_DSV)
	if (dsi_panel_full_power_seq(panel)) {
		pr_err("[DEBUG] dsv chip enable + POWER ON\n");
...
int lge_dsi_panel_post_enable_sw49109(struct dsi_panel *panel)
{
	int rc = 0;
...
void lge_panel_enter_deep_sleep_sw49109(struct dsi_panel *panel)
{
	int rc = 0;
	...
	if (panel->lge.flag_deep_sleep_ctrl) {
		...
		dsi_ctrl_isr_configure(m_ctrl->ctrl, true);
		rc = dsi_display_clk_ctrl(display->dsi_clk_handle, DSI_ALL_CLKS, DSI_CLK_ON);
		rc = dsi_ctrl_update_host_state(m_ctrl->ctrl, DSI_CTRL_OP_HOST_INIT, true);
		rc = dsi_ctrl_set_host_engine_state(m_ctrl->ctrl, DSI_CTRL_ENGINE_ON);
		panel->panel_initialized = true;

		lge_ddic_dsi_panel_tx_cmd_set(panel, LGE_DDIC_DSI_DISP_CTRL_DEEP_SLEEP_ENTER);

		panel->panel_initialized = false;
		rc = dsi_ctrl_update_host_state(m_ctrl->ctrl, DSI_CTRL_OP_HOST_INIT, false);
		rc = dsi_ctrl_update_host_state(m_ctrl->ctrl, DSI_CTRL_OP_HOST_ENGINE, false);
		rc = dsi_display_clk_ctrl(display->dsi_clk_handle, DSI_ALL_CLKS, DSI_CLK_OFF);
		dsi_ctrl_isr_configure(m_ctrl->ctrl, false);
		pr_info("%s done \n", __func__);
	}
}

void lge_panel_exit_deep_sleep_sw49109(struct dsi_panel *panel)
{
	int rc = 0;
	struct dsi_display *display = NULL;
	struct dsi_display_ctrl *m_ctrl;
	...
	if (panel->lge.flag_deep_sleep_ctrl) {
		...
		lge_ddic_dsi_panel_tx_cmd_set(panel, LGE_DDIC_DSI_DISP_CTRL_DEEP_SLEEP_EXIT);
...
static void lge_set_custom_rgb_sw49109(struct dsi_panel *panel, bool send_cmd)
{
	int i = 0;
...
static void lge_display_control_store_sw49109(struct dsi_panel *panel, bool send_cmd)
{
...
static void lge_set_screen_tune_sw49109(struct dsi_panel *panel)
{
	int i;
...
static void lge_set_screen_mode_sw49109(struct dsi_panel *panel, bool send_cmd)
{
	...
	switch (panel->lge.screen_mode) {
		case screen_mode_auto:
			...
			if (panel->lge.cm_preset_step == 2 &&
				...
				mutex_unlock(&panel->panel_lock);
				lge_set_custom_rgb_sw49109(panel, send_cmd);
				mutex_lock(&panel->panel_lock);
			}
		...
		case screen_mode_game:
			...
			mutex_unlock(&panel->panel_lock);
			lge_set_custom_rgb_sw49109(panel, send_cmd);
			lge_set_screen_tune_sw49109(panel);  //saturation update
			mutex_lock(&panel->panel_lock);
			break;
		case screen_mode_expert:
			...
			mutex_unlock(&panel->panel_lock);
			lge_set_custom_rgb_sw49109(panel, send_cmd);
			lge_set_screen_tune_sw49109(panel);
			mutex_lock(&panel->panel_lock);
			break;
		default:
			break;
	...
	mutex_unlock(&panel->panel_lock);
	lge_display_control_store_sw49109(panel, send_cmd);
	return;
}

static void lge_set_ht_lcd_tune_sw49109(struct dsi_panel *panel, int index)
{
	switch(index) {
		case HT_TUNE_MODE_STEP_0:
			pr_info("HT Tune Mode Step 0\n");
			lge_ddic_dsi_panel_tx_cmd_set(panel, LGE_DDIC_DSI_DISP_CTRL_HT_LCD_TUNE_0);
			break;
		case HT_TUNE_MODE_STEP_1:
			pr_info("HT Tune Mode Step 1\n");
			lge_ddic_dsi_panel_tx_cmd_set(panel, LGE_DDIC_DSI_DISP_CTRL_HT_LCD_TUNE_1);
			break;
		case HT_TUNE_MODE_STEP_2:
			pr_info("HT Tune Mode Step 2\n");
			lge_ddic_dsi_panel_tx_cmd_set(panel, LGE_DDIC_DSI_DISP_CTRL_HT_LCD_TUNE_2);
			break;
		case HT_TUNE_MODE_STEP_3:
			pr_info("HT Tune Mode Step 3\n");
			lge_ddic_dsi_panel_tx_cmd_set(panel, LGE_DDIC_DSI_DISP_CTRL_HT_LCD_TUNE_3);
			break;
		case HT_TUNE_MODE_STEP_4:
			pr_info("HT Tune Mode Step 4\n");
			lge_ddic_dsi_panel_tx_cmd_set(panel, LGE_DDIC_DSI_DISP_CTRL_HT_LCD_TUNE_4);
			break;
		default:
...
struct lge_ddic_ops sw49109_ops = {
	.lge_dsi_panel_power_on = lge_panel_power_on_sw49109,
	.lge_dsi_panel_post_enable = lge_dsi_panel_post_enable_sw49109,
	.lge_panel_enter_deep_sleep = lge_panel_enter_deep_sleep_sw49109,
	.lge_panel_exit_deep_sleep = lge_panel_exit_deep_sleep_sw49109,
	.lge_display_control_store = lge_display_control_store_sw49109,
	.lge_set_custom_rgb = lge_set_custom_rgb_sw49109,
	.lge_set_screen_tune = lge_set_screen_tune_sw49109,
	.lge_set_screen_mode = lge_set_screen_mode_sw49109,
	.lge_set_ht_lcd_tune = lge_set_ht_lcd_tune_sw49109,
};
