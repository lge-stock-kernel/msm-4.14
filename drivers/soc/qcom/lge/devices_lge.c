...
char *lge_rev_str[] = {
	...
	"rev_a", "rev_a1", "rev_a2", "rev_b", "rev_b1", "rev_c",
	"rev_c1", "rev_10", "rev_11", "rev_12",
	"rev_13", "reserved"
};
#else
char *lge_rev_str[] = {
...
#ifdef CONFIG_LGE_MME_DRV_SUPPORT
...
static int __init lge_tdmb_hwinfo_setup(char *dtv)
{
    if (!strcmp(dtv, "3")) {
        lge_tdmb_hwinfo = 3;
        pr_info("LGE JFULLSEG Support\n");
    }else if (!strcmp(dtv, "2")) {
        lge_tdmb_hwinfo = 2;
        pr_info("LGE TDMB Support\n");
    }else if (!strcmp(dtv, "1")) {
        lge_tdmb_hwinfo = 1;
        pr_info("LGE LATINDTV Support\n");
    } else if (!strcmp(dtv, "0")) {
        lge_tdmb_hwinfo = 0;
        pr_info("LGE DTV Not Support\n");
    } else {
        lge_tdmb_hwinfo = 0;
        pr_info("Undefined DTV Support Info. Disable DTV\n");
    }
    return 1;
}
