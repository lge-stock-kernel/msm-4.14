...
#include "../../gpu/drm/msm/lge/notifier/lge_dp_notifier.h"
#endif
	...
	case PE_SNK_STARTUP:
		if (pd->current_dr == DR_NONE || pd->current_dr == DR_UFP) {
			...
			if (!ret) {
				...
				if ((usb_compliance_mode && val.intval == POWER_SUPPLY_TYPE_USB_FLOAT) ||
	...
	case SVDM_CMD_TYPE_RESP_ACK:
...
#ifdef CONFIG_LGE_USB
#ifdef CONFIG_LGE_DISPLAY_NOT_SUPPORT_DISPLAYPORT
			if (ID_HDR_PRODUCT_TYPE(vdos[0]) == ID_HDR_PRODUCT_AMA) {
				usbpd_info(&pd->dev, "Alternate Mode Adapter detected!!!\n");
				dp_notify_set_state(&dpdev, 1);
			}
#endif
#endif
		...
		case USBPD_SVDM_DISCOVER_SVIDS:
			...
			if (pd->has_dp) {
				...
				dp_notify_set_state(&dpdev, pd->has_dp);
			}
#endif
			break;
...
#ifdef CONFIG_LGE_USB
			if ((usb_compliance_mode && pd->psy_type == POWER_SUPPLY_TYPE_USB_FLOAT) ||
...
#ifdef CONFIG_LGE_USB
		/* non-PD connected */
		if (usb_compliance_mode && pd->vbus_present)
			pd->hard_reset_count = 3;
#endif
		usbpd_set_state(pd, PE_SNK_STARTUP);
		break;
