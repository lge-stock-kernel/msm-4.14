...
static unsigned long crashlog_mem_addr = 0;
static unsigned long crashlog_mem_size = 0;
static int get_crashlog_mem(struct reserved_mem *rmem)
{
	crashlog_mem_addr = rmem->base;
	crashlog_mem_size = rmem->size;
	return 0;
}

static void release_crashlog_mem(void)
{
	lge_panic_handler_reserve_cleanup(crashlog_mem_addr,crashlog_mem_size);
	crashlog_mem_addr = 0;
	crashlog_mem_size = 0;
}
RESERVEDMEM_OF_DECLARE(crashlog_mem, "crashlog_mem", get_crashlog_mem);
