...
static void update_veneer_uninodes(struct veneer* veneer_me) {
	...
	struct power_supply*	psy = NULL;
	struct power_supply* psy_usb;
	union  power_supply_propval buffer = { .intval = 0, };
	...
	*/
	...
	if (!unified_nodes_show("charger_highspeed", buff) || strncmp(buff, "1", 1)
		...
		|| type == CHARGING_SUPPLY_TYPE_FLOAT || veneer_me->usbin_typefix) {
		if (veneer_me->presence_usb) {
		...
		if(psy_usb) {
			power_supply_get_property(psy_usb, POWER_SUPPLY_PROP_PD_ACTIVE, &buffer);
			pr_veneer("is_pd_active = %d\n", buffer.intval);
			if(buffer.intval)
				unified_nodes_store("charger_highspeed",
					mw_now >= mw_highspeed ? "1" : "0", 1);
			else if (type == CHARGING_SUPPLY_TYPE_FLOAT)
				unified_nodes_store("charger_highspeed", "0", 1);
			else
				unified_nodes_store("charger_highspeed",
					(mw_now >= mw_highspeed && type != CHARGING_SUPPLY_DCP_10K) ? "1" : "0", 1);
		}
	}
	else
...
static void detect_carrier_requirement(/* @Nonnull */ struct veneer* veneer_me)
{
	...
	if(veneer_me->laop_icl_limit_enable)
	{
...
static void psy_external_logging(struct work_struct* work) {
	...
	*/
	...
	schedule_delayed_work(to_delayed_work(work),
		msecs_to_jiffies(debug_polling_time));
}
...
static bool feed_charging_time(int* power, int* rawsoc, int* bsm_ttf) {
	...
	if (veneer_me) {
		...
		*power = (psy && !power_supply_get_property(psy, POWER_SUPPLY_PROP_POWER_NOW, &val))
		...
		psy = get_psy_battery(veneer_me);
		*rawsoc = (psy && !power_supply_get_property(psy, POWER_SUPPLY_PROP_CAPACITY_RAW, &val))
			? val.intval : 0;
...
#ifndef CONFIG_LGE_PM_CCD
	ret &= charging_time_create(of_find_node_by_name(dnode, "charging-time"), fullraw, mincap, feed_charging_time, back_charging_time);
