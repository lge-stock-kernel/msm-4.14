...
//		return 250;
	...
	if (qg->batt_psy) {
		if (tcomp.qnovo_charging) {
			ichg = get_batt_charging_current(qg);
		}
		else {
...
/* Capacity  */ qginfo.capacity_rescaled*10, qginfo.capacity_chargecnt, qginfo.capacity_learned,
...
/* SoCs      */ qginfo.misc_battsoc);//, qginfo.misc_ccsocsw, qginfo.misc_ccsoc);
}
...
/* Temperature */
	...
	qginfo.temp_vts = !thermal_zone_get_temp(tzd, &buf)
		? buf / 100 : -1000;
/* Impedance */
	qginfo.impedance_esr = !qg_sdam_read(SDAM_ESR_CHARGE_DELTA, &buf)
		? buf : LGE_QG_INITVAL;
/* Misc */
#if 0
...
#endif
/* SoCs */
	qginfo.misc_battsoc = !qg_sdam_read(SDAM_SOC, &buf)
		? ((u32)buf >> 24)*1000/255 : LGE_QG_INITVAL;
#if 0
	qginfo.misc_ccsocsw = !qg_get_sram_prop(qg, QG_SRAM_CC_SOC_SW, &buf)
		? div_s64((s64)buf*1000, CC_SOC_30BIT) : LGE_QG_INITVAL;
	qginfo.misc_ccsoc = !qg_get_sram_prop(qg, QG_SRAM_CC_SOC, &buf)
		? div_s64((s64)buf*1000, CC_SOC_30BIT) : LGE_QG_INITVAL;
#endif
	/* logging finally */
