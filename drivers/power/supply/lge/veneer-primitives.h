...
static inline union power_supply_propval vote_make(enum voter_type type, int limit) {
...
static inline enum voter_type vote_type(const union power_supply_propval* vote) {
...
static inline const char* vote_title(enum voter_type type) {
	switch (type) {
...
static inline enum voter_type vote_fromdt(const char* string) {
...
static inline int vote_limit(const union power_supply_propval* vote) {
...
static inline bool charger_fabcable(enum charging_supplier charger) {
...
static inline const char* charger_name(enum charging_supplier charger) {
...
static inline bool veneer_extension_pspoverlap(enum power_supply_property* ori_arr, int ori_cnt,
...
enum veneer_exception { // Can be accumulated to the 'int' storage
	// for battery			0x000?
	EXCEPTION_BATTERY_VBATLOW	= BIT(0),
	...
	EXCEPTION_CHARGING_TIMEOUT	= BIT(8),
	EXCEPTION_CHARGING_AICLFAIL	= BIT(9),
};
