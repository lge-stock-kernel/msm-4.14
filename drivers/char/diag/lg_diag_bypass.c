...
#ifdef CONFIG_DIAGFWD_BRIDGE_CODE
        if(ret > 0 && ctxt == DIAGFWD_MDM) {
            pr_info("%s - diagfwd_bridge_mux_write_done_bypass called\n", __func__);
            diagfwd_bridge_mux_write_done_bypass((unsigned char *)buf, len, ctxt, DIAGFWD_MDM);
            return 1;
        }
#endif
...
#ifdef CONFIG_DIAGFWD_BRIDGE_CODE
    if(is_5g_opened) {
        pr_info( "%s disconnect 5g bypass\n", __func__);
        diagfwd_bridge_mux_disconnect_bypass(DIAGFWD_MDM, DIAG_USB_MODE);
    }
#endif
}
...
static int diag_bypass_request(const unsigned char* buf, int count)
{
    if(diag_bypass_enable) {
#ifdef CONFIG_DIAGFWD_BRIDGE_CODE
        if( strncmp(buf, "MINIOS5G_", 9) == 0 ) {
            if(!is_5g_opened) {
                pr_info( "%s connect 5g bypass\n", __func__);
                diagfwd_bridge_mux_connect_bypass(DIAGFWD_MDM, DIAG_USB_MODE);

                mdelay(50);

                is_5g_opened = 1;
            }
            pr_info( "%s write 5g bypass %d\n", __func__, buf[9]);
            diagfwd_bridge_write(DIAGFWD_MDM, (void*)buf+9, (unsigned)count-9);
        } else {
