...
static unsigned char load_firmware(struct atmf04_data *data, struct i2c_client *client, const char *name)
{
  ...
  PINFO(" ATMF04 SAR controller ATMF04 chip_id read check start\n");
...
#if 1   // atmf04 should be done hw reset at this time ( from adsemicon )
...
/* CS Duty Rnage Check */
    ...
    {
		PINFO("Check_Far IRQ Status NEAR[%d]",gpio_get_value(data->platform_data->irq_gpio));
...
static void atmf04_work_handler(struct work_struct *work)
{
    ...
    else
    {
	    ...
	    if (irq_state == 0 && data->touch_out == (CH2_FAR|CH1_FAR))
	    {
           ...
           PINFO("ATMF04 [%d]FAR CR1:%d,CR2:%d ",data->platform_data->irq_gpio, cs_per_result, cs_per_result_ch2);
	    }
        /* NEAR */
	    else
        {
           ...
           PINFO("ATMF04 [%d]NEAR CR1:%d,CR2:%d ",data->platform_data->irq_gpio, cs_per_result, cs_per_result_ch2);
	    }
        ...
        if(data->touch_out)
        {
			...
			PINFO("ATMF04 [%d]NEAR CR1:%d ",data->platform_data->irq_gpio, cs_per_result);
	    }
        else
        {
            data->cap_detection = 0;
			...
			PINFO("ATMF04 [%d]FAR CR1:%d ",data->platform_data->irq_gpio, cs_per_result);
		}
		PINFO("ATMF04 [%d] Work Handler done",data->platform_data->irq_gpio);
#endif
	}
...
static int __init atmf04_init(void)
{
	...
	if(HW_SKU_KR == lge_get_sku_carrier()) {
		PINFO("sku(%d) skip probe", lge_get_sku_carrier());
		return 0;
	}
#endif
#endif
