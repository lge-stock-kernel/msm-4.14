...
static void prd_param_set(struct device *dev)
{
	...
	param->lpwg_sd_test_set = (1 << U0_M1_RAWDATA_TEST) | (1 << U0_M2_RAWDATA_TEST)
				| (1 << U0_M1_JITTER_TEST) | (1 << U0_M2_JITTER_TEST);
...
static int spec_file_read(struct device *dev)
{
	...
	if (touch_check_boot_mode(dev) >= TOUCH_MINIOS_MFTS_FOLDER) {
		if ((d->ic_info.pt_info.lcm_type >= AFTER_PANEL_REV)
			&& (d->ic_info.pt_info.lot_num >= AFTER_DIC_REV))
			snprintf(path, sizeof(path), "%s", ts->panel_spec_mfts[1]);
		else
			snprintf(path, sizeof(path), "%s", ts->panel_spec_mfts[0]);
	} else {
		if ((d->ic_info.pt_info.lcm_type >= AFTER_PANEL_REV)
			&& (d->ic_info.pt_info.lot_num >= AFTER_DIC_REV))
			snprintf(path, sizeof(path), "%s", ts->panel_spec[1]);
		else
			snprintf(path, sizeof(path), "%s", ts->panel_spec[0]);
	}
	...
	if (request_firmware(&fwlimit, path, dev) < 0) {
...
static int sic_get_limit(struct device *dev, char *breakpoint, u16 (*buf)[COL_SIZE])
{
	...
	boot_mode = touch_check_boot_mode(dev);
	if (boot_mode > TOUCH_MINIOS_MFTS_CURVED || boot_mode < TOUCH_NORMAL_BOOT) {
		ret = -1;
		goto error;
	}
...
static int write_test_mode(struct device *dev, u32 type)
{
	...
	/* TestType Set */
	sw49109_reg_write(dev, TC_PT_TEST_CTL,
			(u32 *)&testmode,
			sizeof(testmode));
	...
	do {
		touch_msleep(50);
		sw49109_reg_read(dev, PT_TEST_STS, (u8 *)&rdata, sizeof(rdata));
		TOUCH_I("rdata = 0x%x\n", rdata);
...
static int prd_read_print_frame(struct device *dev, int offset, int row, int col)
{
	...
	sw49109_write_value(dev, SERIAL_DATA_OFFSET, offset);

	sw49109_reg_read(dev, SERIAL_DATA_ACCESS, (u16 *)data_buf, row*col*sizeof(u16));
...
static void read_tune_code(struct device *dev, u8 type)
{
	...
	sw49109_reg_read(dev, tune_code_addr, &t,
...
static int prd_rawdata_test(struct device *dev, u8 type)
{
	...
	/*result = */
	if ((type == U3_M1_RAWDATA_TEST) || (type == U3_M2_RAWDATA_TEST) ||
		(type == U0_M1_RAWDATA_TEST) || (type == U0_M2_RAWDATA_TEST))
...
static int firmware_version_log(struct device *dev)
{
	...
	if (boot_mode >= TOUCH_MINIOS_MFTS_FOLDER) {
		...
		if (ret < 0) {
			TOUCH_E("failed to read ic info (ret: %d)\n", ret);
			goto out;
		}
	}
...
static int production_info_check(struct device *dev)
{
	...
	if (strncmp(d->ic_info.product_id, "L0W64MH3", 8)) {
		TOUCH_E("Invalid product id : [%s]\n", d->ic_info.product_id);
		ret = -EINVAL;
		goto out;
	}

	TOUCH_I("%s - lcm[%d], lot[%d], fpc[%d]\n", __func__,
			d->ic_info.pt_info.lcm_type, d->ic_info.pt_info.lot_num,
			d->ic_info.pt_info.fpc_type);
...
static ssize_t show_sd(struct device *dev, char *buf)
{
	...
	if (fw_ver_ret < 0) {
		...
		TOUCH_I("========RESULT=======\n");
		ret += snprintf(buf + ret, PAGE_SIZE - ret, "Raw Data : Fail (Check connector)\n");
		TOUCH_I("Raw Data : Fail (Check connector)\n");
		ret += snprintf(buf + ret, PAGE_SIZE - ret, "Channel Status : Fail (Check connector)\n");
	...
	if (prd_info_ret < 0) {
		ret += snprintf(buf + ret, PAGE_SIZE - ret, "\n========RESULT=======\n");
		TOUCH_I("========RESULT=======\n");
		ret += snprintf(buf + ret, PAGE_SIZE - ret, "Raw Data : Fail (Invalid Product Info)\n");
		TOUCH_I("Raw Data : Fail (Invalid Product Info)\n");
		ret += snprintf(buf + ret, PAGE_SIZE - ret, "Channel Status : Fail (Invalid Product Info)\n");
		TOUCH_I("Channel Status : Fail (Invalid Product Info)\n");
		goto out;
	}
	...
	sw49109_write_value(dev, TC_PT_TEST_CTL, pt_command);

	touch_interrupt_control(ts->dev, INTERRUPT_DISABLE);
	sw49109_tc_driving(dev, LCD_MODE_STOP);
	...
	if ((u3_m2_raw_ret + u3_m2_jitter_ret) == 0) {
		ret += snprintf(buf + ret, PAGE_SIZE - ret, "Raw Data : Pass\n");
	} else {
		ret += snprintf(buf + ret, PAGE_SIZE - ret,
				"Raw Data : Fail (m2_raw:%d/m2_jitter:%d)\n",
				u3_m2_raw_ret, u3_m2_jitter_ret);
	}
...
static int start_firmware(struct device *dev)
{
	...
	ret = sw49109_reg_write(dev, ADDR_CMD_REG_SIC_IMAGECTRL_TYPE, (void *)&cmd,
			sizeof(u32));
	if (ret < 0)
		goto error;

	ret = sw49109_reg_read(dev, ADDR_CMD_REG_SIC_IMAGECTRL_TYPE, &check_data,
			sizeof(u32));
	if (ret < 0)
		goto error;
...
static int stop_firmware(struct device *dev, u32 wdata)
{
	...
	/* STOP F/W to check */
	sw49109_reg_write(dev, ADDR_CMD_REG_SIC_IMAGECTRL_TYPE, &wdata, sizeof(u32));
	sw49109_reg_read(dev, ADDR_CMD_REG_SIC_IMAGECTRL_TYPE, &check_data,
			sizeof(u32));
	sw49109_reg_read(dev, ADDR_CMD_REG_SIC_IMAGECTRL_TYPE, &check_data,
			sizeof(u32));
	...
	do {
		...
		sw49109_reg_read(dev, ADDR_CMD_REG_SIC_GETTER_READYSTATUS,
				&read_val, sizeof(u32));
					...
					u8 *buf, int size, u32 cmd, u32 offset, int flag)
{
	...
	ret = sw49109_reg_write(dev, SERIAL_DATA_OFFSET, (u8 *)&offset, sizeof(offset));
	if (ret < 0)
		goto out;

	ret = sw49109_reg_read(dev, SERIAL_DATA_ACCESS, (void *)buf, size);
	if (ret < 0)
		goto out;
...
static ssize_t prd_show_app_info(struct device *dev, char *buf)
{
	...
	TOUCH_I("CMD_T:%d S_SCR_I:%d S_SCR_J:%d\n",
...
static ssize_t get_data(struct device *dev, int16_t *buf, u32 wdata)
{
	...
	case CMD_RAWDATA:
		...
		sw49109_reg_write(dev, SERIAL_DATA_OFFSET,
				(u32 *)&m2_data_offset, sizeof(m2_data_offset));
		sw49109_reg_read(dev, SERIAL_DATA_ACCESS, (int16_t *)buf,
				sizeof(int16_t) * ROW_SIZE * COL_SIZE);
		break;
	case CMD_BASE_DATA:
	...
	case CMD_DELTADATA:
		delta_buf = devm_kzalloc(dev,
				sizeof(u16) * ((COL_SIZE + 2) * (ROW_SIZE + 2)), GFP_KERNEL);
		...
		sw49109_reg_write(dev, SERIAL_DATA_OFFSET,
				(u32 *)&m2_data_offset, sizeof(m2_data_offset));
		sw49109_reg_read(dev, SERIAL_DATA_ACCESS, (s16 *)delta_buf,
		...
		break;
	case CMD_LABELDATA:
		label_buf = devm_kzalloc(dev,
				sizeof(s8) * ((COL_SIZE + 2) * (ROW_SIZE + 2)), GFP_KERNEL);
		...
		sw49109_reg_write(dev, SERIAL_DATA_OFFSET,
				(u32 *)&m2_data_offset, sizeof(m2_data_offset));
		sw49109_reg_read(dev, SERIAL_DATA_ACCESS, (s8 *)label_buf,
...
static ssize_t show_lpwg_sd(struct device *dev, char *buf)
{
	...
	if ((u0_m1_raw_ret + u0_m2_raw_ret + u0_m1_jitter_ret + u0_m2_jitter_ret) == 0) {
		ret += snprintf(buf + ret, PAGE_SIZE - ret, "LPWG RawData : %s\n", "Pass");
	} else {
		ret += snprintf(buf + ret, PAGE_SIZE - ret,
			"LPWG RawData : %s (m1_raw:%d/m2_raw:%d/m1_jitter:%d/m2_jitter:%d)\n", "Fail",
			u0_m1_raw_ret, u0_m2_raw_ret, u0_m1_jitter_ret, u0_m2_jitter_ret);
	}
	...
	if (boot_mode == TOUCH_NORMAL_BOOT)
		touch_interrupt_control(ts->dev, INTERRUPT_ENABLE);
...
int sw49109_prd_register_sysfs(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw49109_data *d = to_sw49109_data(dev);
	struct siw_hal_prd_data *prd = (struct siw_hal_prd_data *)d->prd;
