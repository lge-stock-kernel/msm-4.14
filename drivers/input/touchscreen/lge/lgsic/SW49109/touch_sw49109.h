...
#ifndef LGE_TOUCH_sw49109_H
#define LGE_TOUCH_sw49109_H
...
/* revision */
#define BEFORE_PANEL_REV	(1)
#define BEFORE_DIC_REV		(0)
#define BEFORE_FPC_REV		(1)
#define AFTER_PANEL_REV		(2)
#define AFTER_DIC_REV		(1)
#define AFTER_FPC_REV		(1)

/* device control */
#define TC_VERSION			(0x282)
#define TC_PRODUCT_CODE			(0x283)
#define TC_PRODUCT_ID1			(0x284)
#define TC_PRODUCT_ID2			(0x285)
#define TC_CFG_VERSION			(0x287)
#define IC_STATUS			(0x200)	/* sw49109_touch_info base addr */
...
#define PT_INFO_LCM_TYPE		(0x29C) /* sw49109_pt_info base addr */
#define PT_INFO_LOT_NUM			(0x29D)
#define PT_INFO_FPC_TYPE		(0x29E)
#define PT_INFO_PT_DATE			(0x29F)
#define PT_INFO_PT_TIME			(0x2A0)
#define PT_INFO_CHIP_REV		(0x2A1)
...
#define SWIPE_TIME_MIN_H_CTRL		(0xF23)
#define SWIPE_TIME_MIN_V_CTRL		(0xF24)
#define SWIPE_TIME_MAX_H_CTRL		(0xF25)
#define SWIPE_TIME_MAX_V_CTRL		(0xF26)
#define SWIPE_ACT_AREA_X1_H_CTRL	(0xF27)
#define SWIPE_ACT_AREA_Y1_H_CTRL	(0xF28)
#define SWIPE_ACT_AREA_X2_H_CTRL	(0xF29)
#define SWIPE_ACT_AREA_Y2_H_CTRL	(0xF2A)
#define SWIPE_ACT_AREA_X1_V_CTRL	(0xF2B)
#define SWIPE_ACT_AREA_Y1_V_CTRL	(0xF2C)
#define SWIPE_ACT_AREA_X2_V_CTRL	(0xF2D)
#define SWIPE_ACT_AREA_Y2_V_CTRL	(0xF2E)
#define SWIPE_START_AREA_X1_H_CTRL		(0xF2F)
#define SWIPE_START_AREA_Y1_H_CTRL		(0xF30)
#define SWIPE_START_AREA_X2_H_CTRL		(0xF31)
#define SWIPE_START_AREA_Y2_H_CTRL		(0xF32)
#define SWIPE_START_AREA_X1_V_CTRL		(0xF33)
#define SWIPE_START_AREA_Y1_V_CTRL		(0xF34)
#define SWIPE_START_AREA_X2_V_CTRL		(0xF35)
#define SWIPE_START_AREA_Y2_V_CTRL		(0xF36)
#define SWIPE_WRONG_DIRECTION_THD_CTRL	(0xF37)
#define SWIPE_INIT_RATIO_CHK_DIST_CTRL	(0xF38)
#define SWIPE_INIT_RATIO_THD_CTRL		(0xF39)
...
/* Flexible Report */
#define REPORT_PACKET_SIZE		(12) // 12byte = sizeof(sw49109_touch_data)
...
#define rconf_dn_index			(0x286)	//(0x316)
...
#if defined(CONFIG_LGE_TOUCH_CORE_MTK) || defined(CONFIG_LGE_TOUCH_CORE_QCT)
enum {
	LGD_INCELL_sw49109 = 0,
	TOVIS_INCELL_sw49109,
};
enum {
	LGD_INCELL_sw49109_HD = 0,
	TOVIS_INCELL_sw49109_HD,
};
#endif
...
struct sw49109_touch_debug_info {
...
struct sw49109_touch_info {
	...
	u32 current_mode:3;
	u32 palm_bit:13;
	struct sw49109_touch_data data[10];	//120byte
} __packed;

struct sw49109_tc_version {
	u8 minor_ver;
	u8 major_ver:4;
	u8 build_ver:4;
	u8 chip_id;
...
struct sw49109_ic_info {
	struct sw49109_tc_version version;
	u8 product_id[9];
	struct sw49109_pt_info pt_info;
};

struct sw49109_data {
	struct device *dev;
	struct kobject kobj;
	struct sw49109_touch_info info;
	struct sw49109_touch_debug_info debug_info;
	struct sw49109_ic_info ic_info;
...
int sw49109_debug_info(struct device *dev);
int sw49109_reset_ctrl(struct device *dev, int ctrl);

static inline struct sw49109_data *to_sw49109_data(struct device *dev)
{
	return (struct sw49109_data *)touch_get_device(to_touch_core(dev));
}

static inline struct sw49109_data *to_sw49109_data_from_kobj(struct kobject *kobj)
{
	return (struct sw49109_data *)container_of(kobj,
			struct sw49109_data, kobj);
}
static inline int sw49109_read_value(struct device *dev,
		u16 addr, u32 *value)
{
	return sw49109_reg_read(dev, addr, value, sizeof(*value));
}

static inline int sw49109_write_value(struct device *dev,
		u16 addr, u32 value)
{
	return sw49109_reg_write(dev, addr, &value, sizeof(value));
}
