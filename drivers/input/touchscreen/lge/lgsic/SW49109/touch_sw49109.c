 ...
 */
#include "touch_sw49109.h"
#include "touch_sw49109_prd.h"
...
{
	struct sw49109_data *d = to_sw49109_data(dev);
...
int sw49109_reg_read(struct device *dev, u16 addr, void *data, int size)
{
...
int sw49109_reg_write(struct device *dev, u16 addr, void *data, int size)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw49109_data *d = to_sw49109_data(dev);
	struct touch_bus_msg msg = {0, };
...
static int sw49109_drm_notifier_callback(struct notifier_block *self,
		unsigned long event, void *data)
{
	struct touch_core_data *ts =
		container_of(self, struct touch_core_data, drm_notif);
	struct sw49109_data *d = to_sw49109_data(ts->dev);
...
static int sw49109_fb_notifier_callback(struct notifier_block *self,
		unsigned long event, void *data)
{
...
static int sw49109_sw_reset(struct device *dev, int mode)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw49109_data *d = to_sw49109_data(dev);
	int ret = 0;
	...
	if (mode == SW_RESET) {
		...
		sw49109_write_value(dev, sys_rst_ctl, 2);
		touch_msleep(20);
		sw49109_write_value(dev, sys_rst_ctl, 0);
	} else if (mode == SW_RESET_CODE_DUMP) {
		/* Touch F/W resister reset and do code flash dump */
		sw49109_write_value(dev, sys_rst_ctl, 1);
		touch_msleep(20);
		sw49109_write_value(dev, sys_rst_ctl, 0);
	} else {
...
int sw49109_hw_reset(struct device *dev, int mode)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw49109_data *d = to_sw49109_data(dev);
...
int sw49109_reset_ctrl(struct device *dev, int ctrl)
{
	TOUCH_TRACE();
	...
	case SW_RESET_CODE_DUMP:
		sw49109_sw_reset(dev, ctrl);
		break;

	case POWER_HW_RESET_ASYNC:
	case POWER_HW_RESET_SYNC:
		sw49109_hw_reset(dev, ctrl);
		break;
	default:
		break;
	}
...
int sw49109_display_power_ctrl(struct device *dev, int ctrl)
{
	TOUCH_TRACE();
...
static int sw49109_power(struct device *dev, int ctrl)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw49109_data *d = to_sw49109_data(dev);
	...
	case POWER_HW_RESET_SYNC:
		...
		break;
	case POWER_SW_RESET:
		sw49109_reset_ctrl(dev, SW_RESET);
		break;
	case POWER_SLEEP:
		if (d->p_param.deep_sleep_power_control == FUNC_ON)
			sw49109_display_power_ctrl(dev, DEEP_SLEEP_ENTER);
		break;
	case POWER_WAKE:
		if (d->p_param.deep_sleep_power_control == FUNC_ON)
			sw49109_display_power_ctrl(dev, DEEP_SLEEP_EXIT);
		break;
	case POWER_DSV_TOGGLE:
		if (d->p_param.dsv_toggle == FUNC_ON)
			sw49109_display_power_ctrl(dev, DSV_TOGGLE);
		break;
	case POWER_DSV_ALWAYS_ON:
		if (d->p_param.dsv_toggle == FUNC_ON)
			sw49109_display_power_ctrl(dev, DSV_ALWAYS_ON);
		break;
	default:
...
static void sw49109_get_tci_info(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static void sw49109_get_swipe_info(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int mm_to_point = 15;	/* 1 mm -> about X point */
...
int sw49109_ic_info(struct device *dev)
{
	struct sw49109_data *d = to_sw49109_data(dev);
//	struct touch_core_data *ts = to_touch_core(dev);
	int ret = 0;
...
//	u32 ic_status = 0;
	...
	ret = sw49109_reg_read(dev, TC_VERSION, &d->ic_info.version,
	...
	ret = sw49109_reg_read(dev, TC_PRODUCT_ID1, &d->ic_info.product_id,
	...
	ret = sw49109_reg_read(dev, PT_INFO_LCM_TYPE, &d->ic_info.pt_info,
				sizeof(d->ic_info.pt_info));
	if (ret < 0) {
		TOUCH_E("PT_INFO_LCM_TYPE read failed : %d.\n", ret);
		return ret;
	}
	ret = sw49109_reg_read(dev, SPR_BOOT_ST, &bootmode, sizeof(bootmode));
	if (ret < 0) {
...
/*
	...
	if (((d->ic_info.version.chip_id != d->p_param.chip_id) || (d->ic_info.version.protocol_ver != d->p_param.protocol))
		...
		if(d->boot_err_cnt > 5) {
			...
			sw49109_reset_ctrl(dev, POWER_HW_RESET_ASYNC);
		}
	}

	// Error handling when interrupt is low after ic init
	if (gpio_get_value(ts->int_pin) == 0) {
		ret = sw49109_reg_read(dev, IC_STATUS, &ic_status, sizeof(ic_status));
		if (ret < 0) {
...
static int sw49109_setup_q_sensitivity(struct device *dev, int enable)
{
	struct sw49109_data *d = to_sw49109_data(dev);
	struct touch_core_data *ts = to_touch_core(dev);
	int ret = 0;
	...
	ret = sw49109_reg_write(dev, COVER_SENSITIVITY_CTRL, &d->q_sensitivity, sizeof(u32));
...
static int sw49109_get_tci_data(struct device *dev, int count)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw49109_data *d = to_sw49109_data(dev);
	u8 i = 0;
...
static int sw49109_get_swipe_data(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw49109_data *d = to_sw49109_data(dev);
	u32 rdata[3];
...
static int sw49109_lpwg_failreason_ctrl(struct device *dev, int onoff)
{
	int ret = 0;

	TOUCH_I("%s - Failreason %s\n", __func__, onoff ? "Enable" : "Disable");

	ret = sw49109_reg_write(dev, LPWG_FAILREASON_ON_CTRL, &onoff, sizeof(onoff));

	return ret;
}

static int sw49109_tci_active_area(struct device *dev,
		u32 x1, u32 y1, u32 x2, u32 y2)
{
	...
	ret = sw49109_reg_write(dev, TCI_ACTIVE_AREA_X1_CTRL, &active_area[0],
				sizeof(active_area));

	return ret;
}

static int sw49109_tci_active_area_set(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int ret = 0;
...
/* check build error (not use)
		ret = sw49109_tci_active_area(dev,
...
static int sw49109_tci_command(struct device *dev, int cmd)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static int sw49109_tci_knock(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw49109_data *d = to_sw49109_data(dev);
	struct tci_info *info1 = &ts->tci.info[TCI_1];
	...
	ret = sw49109_reg_write(dev, TCI_ENABLE_CTRL, &lpwg_data[0], sizeof(lpwg_data));

	ret = sw49109_tci_command(dev, ACTIVE_AREA_CTRL);
	ret = sw49109_lpwg_failreason_ctrl(dev, d->lpwg_failreason_ctrl);

	return ret;
}

#ifdef LG_KNOCK_CODE
static int sw49109_tci_password(struct device *dev)
{
	return sw49109_tci_knock(dev);
}
#endif

static int sw49109_tci_control(struct device *dev, int mode)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct tci_info *info1 = &ts->tci.info[TCI_1];
	int ret = 0;
	...
	if (mode <= LPWG_PASSWORD_ONLY)
		TOUCH_I("sw49109_tci_control mode = %s\n", mode_str[mode]);

	switch (mode) {
	case LPWG_NONE:
		ts->tci.mode = 0;
		ret = sw49109_tci_command(dev, ENABLE_CTRL);
		break;
	case LPWG_DOUBLE_TAP:
		ts->tci.mode = 0x01;
		info1->intr_delay = 0;
		info1->tap_distance = 10;
		ret = sw49109_tci_knock(dev);
		break;
#ifdef LG_KNOCK_CODE
	case LPWG_PASSWORD:
...
static int sw49109_swipe_control(struct device *dev, u8 lcd_mode)
{
	struct touch_core_data *ts = to_touch_core(dev);
	...
	if (lcd_mode == LCD_MODE_U3) {
		...
		ret = sw49109_reg_write(dev, SWIPE_ON_CTRL,
		...
		ret = sw49109_reg_write(dev, SWIPE_ON_CTRL, &buf, sizeof(buf));
		if (ret < 0)
...
static int sw49109_swipe_enable(struct device *dev, bool enable)
{
	struct sw49109_data *d = to_sw49109_data(dev);
	...
	if (enable)
		sw49109_swipe_control(dev, d->lcd_mode);

	return 0;
}

static int sw49109_clock(struct device *dev, int state)
{
	int ret = 0;
	int onoff = (state == IC_NORMAL) ? 1 : 0;

	ret = sw49109_reg_write(dev, SPI_OSC_CTL, &onoff, sizeof(onoff));
	ret = sw49109_reg_write(dev, SPI_CLK_CTL, &onoff, sizeof(onoff));
...
int sw49109_tc_driving(struct device *dev, int mode)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw49109_data *d = to_sw49109_data(dev);
	u32 ctrl = 0;
	...
	ret = sw49109_reg_read(dev, SPR_SUBDISP_ST, &rdata, sizeof(rdata));
	TOUCH_I("DDI Display Mode = 0x%04x\n", rdata);
	ret = sw49109_reg_write(dev, TC_DRIVING_CTL, &ctrl, sizeof(ctrl));
	TOUCH_I("sw49109_tc_driving = 0x%x, 0x%x\n", mode, ctrl);

out:
	return ret;
}

static int sw49109_deep_sleep_ctrl(struct device *dev, int state)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw49109_data *d = to_sw49109_data(dev);
	int ret = 0;
	...
	if (!atomic_read(&ts->state.incoming_call)) {	/* IDLE status */
		...
		if (state == IC_NORMAL) {
			...
			ret = sw49109_setup_q_sensitivity(dev, 0);
			ret = sw49109_tci_control(dev, ts->lpwg.mode);
			ret = sw49109_swipe_enable(dev, true);
			ret = sw49109_tc_driving(dev, d->lcd_mode);
		} else {	/* IC_DEEP_SLEEP */
			ret = sw49109_tc_driving(dev, LCD_MODE_STOP);
			atomic_set(&ts->state.sleep, IC_DEEP_SLEEP);
			touch_interrupt_control(ts->dev, INTERRUPT_DISABLE);
...
static void sw49109_lpwg_failreason(struct device *dev)
{
	struct sw49109_data *d = to_sw49109_data(dev);
	u32 type = 0;
	u32 buf = 0;
	...
	sw49109_reg_read(dev, LPWG_FAILREASON_STS_CTRL, &type, sizeof(type));
	TOUCH_I("[LPWG_FAILREASON_STS_CTRL = [0x%x]\n", type);

	if (type & 0x1) { //TCI-0 Knock-on
		sw49109_reg_read(dev, TCI_FAILREASON_BUF, &buf, sizeof(buf));
		buf = (buf & 0xFFFF);
	...
	if (type & (0x1 << 2)) { //Swipe
		sw49109_reg_read(dev, SWIPE_FAILREASON_BUF, &buf, sizeof(buf));
		buf = (buf & 0xFF);
...
static int sw49109_lpwg_mode(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw49109_data *d = to_sw49109_data(dev);
	int ret = 0;
	...
	if (atomic_read(&ts->state.fb) == FB_SUSPEND) {
		if (ts->mfts_lpwg) {
			ret = sw49109_tci_control(dev, LPWG_DOUBLE_TAP);
			ret = sw49109_swipe_enable(dev, true);
			ret = sw49109_tc_driving(dev, d->lcd_mode);
			return 0;
		}
		...
		if (ts->lpwg.screen) {
			...
			if (atomic_read(&ts->state.sleep) != IC_DEEP_SLEEP)
				ret = sw49109_deep_sleep_ctrl(dev, IC_DEEP_SLEEP);
			...
			TOUCH_I("Knock-on mode == LPWG_NONE & swipe[U/L2/R2].enable == SWIPE_NONE & lcd mode == LCD_MODE_U0\n");
			ret = sw49109_deep_sleep_ctrl(dev, IC_DEEP_SLEEP);
		} else {
			/* knock on/code */
			if (atomic_read(&ts->state.sleep) == IC_DEEP_SLEEP) {
				ret = sw49109_deep_sleep_ctrl(dev, IC_NORMAL);
			} else {
				ret = sw49109_setup_q_sensitivity(dev, 0);
				ret = sw49109_tci_control(dev, ts->lpwg.mode);
				ret = sw49109_swipe_enable(dev, true);
	...
	if (ts->lpwg.screen) {
		...
		ret = sw49109_tci_control(dev, LPWG_NONE);
		ret = sw49109_swipe_enable(dev, true);
		ret = sw49109_setup_q_sensitivity(dev, 0);
		if (ts->lpwg.qcover == HALL_NEAR)
			ret = sw49109_tc_driving(dev, LCD_MODE_U3_QUICKCOVER);
		else
			ret = sw49109_tc_driving(dev, d->lcd_mode);
	} else if (ts->lpwg.sensor == PROX_NEAR) {
		...
		ret = sw49109_tci_control(dev, ts->lpwg.mode);
		ret = sw49109_swipe_enable(dev, true);
		ret = sw49109_tc_driving(dev, LCD_MODE_U3_PARTIAL);
	}

	return ret;
}

static int sw49109_lpwg(struct device *dev, u32 code, void *param)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int *value = (int *)param;
	int ret = 0;
...
static void sw49109_connect(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw49109_data *d = to_sw49109_data(dev);
	...
	sw49109_reg_write(dev, SPECIAL_CHARGER_INFO_CTRL, &d->charger, sizeof(u32));
}

static void sw49109_lcd_mode(struct device *dev, u32 mode)
{
	struct sw49109_data *d = to_sw49109_data(dev);
...
static int sw49109_usb_status(struct device *dev, u32 mode)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static int sw49109_wireless_status(struct device *dev, u32 onoff)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static int sw49109_earjack_status(struct device *dev, u32 onoff)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static void sw49109_fb_notify_work_func(struct work_struct *fb_notify_work)
{
	struct sw49109_data *d = container_of(to_delayed_work(fb_notify_work),
			struct sw49109_data, fb_notify_work);
	int ret = 0;
...
static int sw49109_notify_etc(struct device *dev, ulong event, void *data)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw49109_data *d = to_sw49109_data(dev);
	int ret = 0;
...
static int sw49109_notify_normal(struct device *dev, ulong event, void *data)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw49109_data *d = to_sw49109_data(dev);
	...
	case LCD_EVENT_LCD_MODE:
		...
		sw49109_lcd_mode(dev, *(u32 *)data);
		queue_delayed_work(ts->wq, &d->fb_notify_work, 0);
		break;
	case NOTIFY_CONNECTION:
		TOUCH_I("NOTIFY_CONNECTION!\n");
		ret = sw49109_usb_status(dev, *(u32 *)data);
		break;
	case NOTIFY_WIRELESS:
		TOUCH_I("NOTIFY_WIRELESS!\n");
		ret = sw49109_wireless_status(dev, *(u32 *)data);
		break;
	case NOTIFY_EARJACK:
		TOUCH_I("NOTIFY_EARJACK!\n");
		ret = sw49109_earjack_status(dev, *(u32 *)data);
		break;
	case NOTIFY_IME_STATE:
		TOUCH_I("NOTIFY_IME_STATE!\n");
		ret = sw49109_reg_write(dev, SPECIAL_IME_STATUS_CTRL,
					(u32 *)data, sizeof(u32));
		break;
	case NOTIFY_CALL_STATE:
		...
		if (*(u32 *)data >= INCOMING_CALL_IDLE && *(u32 *)data <= INCOMING_CALL_LTE_OFFHOOK) {
			...
			ret = sw49109_reg_write(dev, SPECIAL_CALL_INFO_CTRL,
						(u32 *)data, sizeof(u32));
		}
		break;
...
static int sw49109_notify(struct device *dev, ulong event, void *data)
{
	...
	if (boot_mode == TOUCH_CHARGER_MODE
		...
		return sw49109_notify_etc(dev, event, data);
	}

	return sw49109_notify_normal(dev, event, data);
}

static int sw49109_init_pm(struct device *dev)
{
	return 0;
}

static void sw49109_init_works(struct sw49109_data *d)
{
	INIT_DELAYED_WORK(&d->fb_notify_work, sw49109_fb_notify_work_func);
}

static void sw49109_init_locks(struct sw49109_data *d)
{
	mutex_init(&d->io_lock);
}

static int sw49109_probe(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw49109_data *d = NULL;
	int ret = 0;
	int boot_mode = TOUCH_NORMAL_BOOT;
	...
	/*******************************************************
	 * Display driver does control the power in sw49109 IC *
	...
	sw49109_init_works(d);
	sw49109_init_locks(d);
	...
	sw49109_get_tci_info(dev);
	sw49109_get_swipe_info(dev);

	d->lcd_mode = LCD_MODE_U3;
	d->lpwg_failreason_ctrl = LPWG_FAILREASON_DISABLE;
...
static int sw49109_remove(struct device *dev)
{
	TOUCH_TRACE();
...
static int sw49109_shutdown(struct device *dev)
{
	TOUCH_TRACE();
...
static int sw49109_fw_compare(struct device *dev, const struct firmware *fw)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw49109_data *d = to_sw49109_data(dev);
...
static int sw49109_condition_wait(struct device *dev,
		u16 addr, u32 *value, u32 expect, u32 mask, u32 delay, u32 retry)
{
	...
	do {
		touch_msleep(delay);
		sw49109_read_value(dev, addr, &data);
...
int sw49109_specific_header_verify(unsigned char *header, int i)
{
...
int sw49109_common_header_verify(t_cfg_info_def *header)
{
...
static int sw49109_img_binary_verify(struct device *dev, unsigned char *imgBuf)
{
	struct sw49109_data *d = to_sw49109_data(dev);
	int flash_fw_size = d->p_param.flash_fw_size;
	...
	if (sw49109_common_header_verify(head) < 0) {
	...
	for (i = 0; i < head->cfg_specific_cnt; i++) {
		if (sw49109_specific_header_verify(specific_ptr, i) < 0) {
...
static int sw49109_fw_upgrade(struct device *dev, const struct firmware *fw)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw49109_data *d = to_sw49109_data(dev);
	...
	if (d->p_param.fw_cfg_use_en == FUNC_ON) {
		img_check_result = sw49109_img_binary_verify(dev, (unsigned char *)fwdata);
	...
	//sw49109_write_value(dev, 0x15, 0);

	/* Reset Touch CM3 core and put system on hold */
	sw49109_write_value(dev, sys_rst_ctl, 2);

	/* sram write enable */
	sw49109_reg_read(dev, sys_sram_ctl, &sys_sram_val, sizeof(sys_sram_val));
	sys_sram_val = (sys_sram_val | 0x1);
	sw49109_write_value(dev, sys_sram_ctl, sys_sram_val);

	/* Write F/W Code to CODE SRAM (80KB) */
	for (i = 0 ; i < flash_fw_size ; i += MAX_RW_SIZE) {
		/* Set code sram base address write */
		sw49109_write_value(dev, spr_code_offset, i / 4);

		/* firmware image download to code sram */
		sw49109_reg_write(dev, code_access_addr, &fwdata[i], MAX_RW_SIZE);
	}

	/* init boot code write */
	sw49109_write_value(dev, fw_boot_code_addr, FW_BOOT_LOADER_INIT);

	/* Start CM3 Boot after Code Dump */
	sw49109_write_value(dev, sys_boot_ctl, 1);

	/* Release Touch CM3 core reset*/
	sw49109_write_value(dev, sys_rst_ctl, 0);

	/* sram write disable */
	//sw49109_write_value(dev, sys_sram_ctl, 0);

	/* Check F/W Boot Done Status */
	ret = sw49109_condition_wait(dev, fw_boot_code_addr, NULL,
				FW_BOOT_LOADER_CODE, 0xFFFFFFFF, 10, 200);
	...
	sw49109_write_value(dev, tc_flash_dn_ctl, (FLASH_KEY_CODE_CMD << 16) | 1);
	touch_msleep(ts->caps.hw_reset_delay);

	/* Check F/W Code Flash Download Status */
	ret = sw49109_condition_wait(dev, tc_flash_dn_sts, &data,
	...
	if (d->p_param.fw_cfg_use_en == FUNC_ON) {
		if (img_check_result == E_FW_CODE_AND_CFG_VALID) {
			...
			sw49109_reg_read(dev, rconf_dn_index, (u8 *)&conf_specific_dn_index, sizeof(u32));
			...
			sw49109_write_value(dev, spr_data_offset, rcfg_c_sram_oft);

			/* Conf data download to conf sram */
			sw49109_reg_write(dev, data_access_addr,
					&fwdata[flash_fw_size], cfg_c_size);

			/* cfg_s sram base address write */
			sw49109_write_value(dev, spr_data_offset, rcfg_s_sram_oft);
			...
			/* Conf Download Start */
			sw49109_write_value(dev, tc_flash_dn_ctl,
			...
			ret = sw49109_specific_header_verify(&fwdata[flash_fw_size + cfg_c_size + (conf_specific_dn_index - 1) * cfg_s_size],
...
static int sw49109_upgrade(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw49109_data *d = to_sw49109_data(dev);
	...
	if (ts->test_fwpath[0]) {
		...
		if ((d->ic_info.pt_info.lcm_type >= AFTER_PANEL_REV)
			&& (d->ic_info.pt_info.lot_num >= AFTER_DIC_REV))
	...
	if (sw49109_fw_compare(dev, fw)) {
		ret = -EINVAL;
		touch_msleep(200);

		for (i = 0; i < 2 && ret < 0; i++)
			ret = sw49109_fw_upgrade(dev, fw);
...
static int sw49109_esd_recovery(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static int sw49109_suspend(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw49109_data *d = to_sw49109_data(dev);
	int mfts_mode = 0;
...
static int sw49109_resume(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int mfts_mode = 0;
	int boot_mode = TOUCH_NORMAL_BOOT;
	...
	mfts_mode = touch_check_boot_mode(dev);
	if ((mfts_mode >= TOUCH_MINIOS_MFTS_FOLDER) && !ts->mfts_lpwg) {
		sw49109_power(dev, POWER_ON);
		touch_msleep(ts->caps.hw_reset_delay);
		ret = sw49109_ic_info(dev);
		if (ret < 0) {
...
static int sw49109_init(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw49109_data *d = to_sw49109_data(dev);
	u32 data = 1;
	...
	if (atomic_read(&ts->state.core) == CORE_PROBE) {
		...
		ts->drm_notif.notifier_call = sw49109_drm_notifier_callback;
		msm_drm_register_client(&ts->drm_notif);
#elif defined(CONFIG_FB)
		TOUCH_I("fb_notif change\n");
		fb_unregister_client(&ts->fb_notif);
		ts->fb_notif.notifier_call = sw49109_fb_notifier_callback;
	...
	ret = sw49109_ic_info(dev);
	if (ret < 0) {
		TOUCH_E("sw49109_ic_info failed, ret:%d\n", ret);
		return ret;
	}

	ret = sw49109_reg_write(dev, TC_DEVICE_CTL, &data, sizeof(data));
	if (ret < 0) {
		TOUCH_E("failed to write \'tc_device_ctrl\', ret:%d\n", ret);
		return ret;
	}

	ret = sw49109_reg_write(dev, TC_INTERRUPT_CTL, &data, sizeof(data));
	if (ret < 0) {
	...
	if (d->p_param.dynamic_tune_table == FUNC_ON) {
		ret = sw49109_reg_write(dev, SPECIAL_DYNAMIC_TUNE_TABLE_CTRL, &d->select_tune_table, sizeof(u8));
		if (ret < 0)
	...
	ret = sw49109_reg_write(dev, SPECIAL_CHARGER_INFO_CTRL, &d->charger, sizeof(u32));
	if (ret < 0)
		TOUCH_E("failed to write \'SPECIAL_CHARGER_CTRL\', ret:%d\n", ret);

	data = atomic_read(&ts->state.ime);
	ret = sw49109_reg_write(dev, SPECIAL_IME_STATUS_CTRL, &data, sizeof(data));
	if (ret < 0)
...
int sw49109_check_status(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw49109_data *d = to_sw49109_data(dev);
	int ret = 0;
...
static void sw49109_palm_log(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw49109_data *d = to_sw49109_data(dev);
...
static int sw49109_irq_abs_data(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw49109_data *d = to_sw49109_data(dev);
	struct sw49109_touch_data *data = d->info.data;
	...
	if (d->p_param.palm_log == FUNC_ON) {
		d->pcount = palm_index;
		sw49109_palm_log(dev);
	}

	return ret;
}

int sw49109_irq_abs(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw49109_data *d = to_sw49109_data(dev);
	...
	return sw49109_irq_abs_data(dev);
}

int sw49109_irq_lpwg(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw49109_data *d = to_sw49109_data(dev);
	int ret = 0;
	...
	if (d->info.wakeup_type == KNOCK_1) {
		if (ts->lpwg.mode != LPWG_NONE) {
			sw49109_get_tci_data(dev, ts->tci.info[TCI_1].tap_count);
			ts->intr_status = TOUCH_IRQ_KNOCK;
		}
	} else if (d->info.wakeup_type == KNOCK_2) {
		if (ts->lpwg.mode >= LPWG_PASSWORD) {
			sw49109_get_tci_data(dev, ts->tci.info[TCI_2].tap_count);
			ts->intr_status = TOUCH_IRQ_PASSWD;
		}
	} else if (d->info.wakeup_type == SWIPE_UP) {
		TOUCH_I("SWIPE_UP\n");
		sw49109_get_swipe_data(dev);
...
int sw49109_irq_debug(struct device *dev)
{
	struct sw49109_data *d = to_sw49109_data(dev);
	int ret = 0, i = 0;
	int count = 0;

	ret = sw49109_reg_read(dev, DEBUG_INFO_ADDR, &d->debug_info, sizeof(d->debug_info));
	if (ret < 0)
		goto error;

	TOUCH_D(ABS, "%s : debug_info.type: %x, debug_info.length: %x\n", __func__,
			d->debug_info.type, d->debug_info.length);
...
int sw49109_irq_read_data(struct device *dev)
{
	struct sw49109_data *d = to_sw49109_data(dev);
	int ret = 0;
	u8 intr_type_temp = 0;
	...
	if (d->p_param.flex_report) {
		ret = sw49109_reg_read(dev, IC_STATUS, &d->info,
				sizeof(d->info) - (REPORT_PACKET_SIZE * REPORT_PACKET_EXTRA_COUNT));
		TOUCH_D(ABS, "Base Read - Addr: %x, ic: %x, tc:%x, Dst: %p, Touch_cnt: %d, Size: %lu\n",
		...
		if (intr_type_temp == INTR_TYPE_REPORT_PACKET) {
			if (d->info.touch_cnt > REPORT_PACKET_BASE_COUNT && d->info.touch_cnt <= MAX_FINGER) {
				ret = sw49109_reg_read(dev, REPORT_PACKET_EXTRA_DATA,
		...
		ret = sw49109_reg_read(dev, IC_STATUS, &d->info, sizeof(d->info));
		if (ret < 0) {
			TOUCH_E("Register read fail\n");
			d->err_cnt++;
			ret = -EGLOBALRESET;
...
int sw49109_irq_handler(struct device *dev)
{
	struct sw49109_data *d = to_sw49109_data(dev);
	int ret = 0;

	ret = sw49109_irq_read_data(dev);
	if (ret < 0)
		goto error;

	ret = sw49109_check_status(dev);
...
static ssize_t store_reg_ctrl(struct device *dev, const char *buf, size_t count)
{
	...
	if (!strcmp(command, "write")) {
		data = value;
		if (sw49109_reg_write(dev, reg_addr, &data, sizeof(u32)) < 0)
			TOUCH_E("reg addr 0x%x write fail\n", reg_addr);
		else
			TOUCH_I("reg[%x] = 0x%x\n", reg_addr, data);
	} else if (!strcmp(command, "read")) {
		if (sw49109_reg_read(dev, reg_addr, &data, sizeof(u32)) < 0)
...
static ssize_t show_lpwg_failreason(struct device *dev, char *buf)
{
	...
	if (sw49109_reg_read(dev, LPWG_FAILREASON_ON_CTRL, (u8 *)&rdata, sizeof(rdata)) < 0) {
...
static ssize_t store_q_sensitivity(struct device *dev, const char *buf, size_t count)
{
	...
	TOUCH_D(QUICKCOVER, "%s: change sensitivity %d -> %d", __func__,
	...
	sw49109_reg_write(dev, COVER_SENSITIVITY_CTRL, &d->q_sensitivity, sizeof(u32));
...
static ssize_t store_select_tune_table(struct device *dev, const char *buf, size_t count)
{
	...
	if (value >= NEW_SAMPLE_TUNE_TABLE && value <= OLD_SAMPLE_TUNE_TABLE) {
		ret = sw49109_reg_write(dev, SPECIAL_DYNAMIC_TUNE_TABLE_CTRL, &value, sizeof(value));
		if (ret < 0) {
			TOUCH_E("failed to write \'SPECIAL_DYNAMIC_TUNE_TABLE_CTRL\', ret:%d\n", ret);
			return count;
		}
...
static struct attribute *sw49109_attribute_list[] = {
...
static const struct attribute_group sw49109_attribute_group = {
	.attrs = sw49109_attribute_list,
};

static int sw49109_register_sysfs(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int ret = 0;
	...
	ret = sysfs_create_group(&ts->kobj, &sw49109_attribute_group);
	if (ret < 0) {
		TOUCH_E("sw49109 sysfs register failed\n");

		goto error;
	}

	sw49109_prd_register_sysfs(dev);
	if (ret < 0) {
		TOUCH_E("sw49109 register failed\n");
...
static int sw49109_get_cmd_version(struct device *dev, char *buf)
{
	struct sw49109_data *d = to_sw49109_data(dev);
	int offset = 0;
...
static int sw49109_get_cmd_atcmd_version(struct device *dev, char *buf)
{
	struct sw49109_data *d = to_sw49109_data(dev);
	int offset = 0;
...
static int sw49109_set(struct device *dev, u32 cmd, void *input, void *output)
{
	TOUCH_TRACE();
...
static int sw49109_get(struct device *dev, u32 cmd, void *input, void *output)
{
	int ret = 0;
	...
	switch (cmd) {
	case CMD_VERSION:
		ret = sw49109_get_cmd_version(dev, (char *)output);
		break;
	case CMD_ATCMD_VERSION:
		ret = sw49109_get_cmd_atcmd_version(dev, (char *)output);
		break;
	default:
		break;
	}
...
static struct touch_driver touch_driver = {
	.probe = sw49109_probe,
	.remove = sw49109_remove,
	.shutdown = sw49109_shutdown,
	.suspend = sw49109_suspend,
	.resume = sw49109_resume,
	.init = sw49109_init,
	.irq_handler = sw49109_irq_handler,
	.power = sw49109_power,
	.upgrade = sw49109_upgrade,
	.esd_recovery = sw49109_esd_recovery,
	.lpwg = sw49109_lpwg,
	.swipe_enable = sw49109_swipe_enable,
	.notify = sw49109_notify,
	.init_pm = sw49109_init_pm,
	.register_sysfs = sw49109_register_sysfs,
	.set = sw49109_set,
	.get = sw49109_get,
};

#define MATCH_NAME	"lge,sw49109"
...
static int __init touch_device_init(void)
{
	...
	if (!is_ddic_name("sw49109")) {
		TOUCH_I("%s, sw49109 not found.\n", __func__);
		return 0;
	}

	TOUCH_I("%s, sw49109 found.\n", __func__);
