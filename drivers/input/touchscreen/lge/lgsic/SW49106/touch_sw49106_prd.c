...
static u16 open_buf[ROW_SIZE*COL_SIZE];
...
static int write_test_mode(struct device *dev, u8 type)
{
	...
	/* TestType Set */
	sw49106_reg_write(dev, tc_tsp_test_ctl,
			(u8 *)&testmode,
			sizeof(testmode));
	...
	do {
		touch_msleep(50);
		sw49106_reg_read(dev, tc_tsp_test_sts,
				(u8 *)&rdata,
				sizeof(rdata));
		TOUCH_I("rdata = 0x%x\n", rdata);
...
*/
	...
	case OPEN_NODE_TEST:
		...
		sw49106_reg_write(dev, tc_tsp_test_data_offset,
							(u8 *)&open_test_offset[2], sizeof(u16));
		sw49106_reg_read(dev, tc_tsp_data_access_addr,
...
static int sdcard_spec_file_read(struct device *dev)
{
	int ret = 0;
	int fd = 0;
	char *path[2] = { "/mnt/sdcard/mh_limit.txt",
		"/mnt/sdcard/mh_limit_mfts.txt"
	};
	int path_idx = 0;
...
static int sic_get_limit(struct device *dev, char *breakpoint, u16 (*buf)[COL_SIZE])
{
	...
	boot_mode = touch_check_boot_mode(dev);
	if (boot_mode > TOUCH_MINIOS_MFTS_CURVED
...
static int prd_open_short_test(struct device *dev)
{
	...
	sw49106_reg_read(dev, tc_tsp_test_pf_result,
...
static void prd_read_rawdata(struct device *dev, u8 type)
{
	...
	sw49106_reg_read(dev, m2_raw_data_offset,
...
static int prd_print_rawdata(struct device *dev, char *buf, int buf_size, u8 type)
{
           int i = 0, j = 0, k = 0;
           int ret = 0;
           int min = 9999;
...
static int prd_compare_rawdata(struct device *dev, u8 type)
{
           ...
           if (type == U0_M1_RAWDATA_TEST) {
                     for (i = 0; i < ROW_SIZE; i++) {
                                for (j = 0; j < col_size; j++) {
                                          TOUCH_I("[%d] [%d] [%d] \n", m1_raw_buf[i * col_size+j], LowerImage[i][j], UpperImage[i][j]);
                     ...
                     for (k =0; k < M1_M2_RAWDATA_TEST_CNT; k++) {
                                for (i = 0; i < ROW_SIZE; i++) {
                                          for (j = 0; j < col_size; j++) {
                                                     TOUCH_I("[%d] [%d] [%d] \n", m2_raw_buf[k][i*col_size + j], LowerImage[i][j], UpperImage[i][j]);
...
static void read_tune_code(struct device *dev, u8 type)
{
	...
	sw49106_reg_read(dev, tune_result_offset,
...
static int prd_rawdata_test(struct device *dev, u8 type)
{
	...
	result = prd_print_rawdata(dev, W_Buf, BUF_SIZE, type);
	//write_file(dev, W_Buf, TIME_INFO_SKIP);
	memset(W_Buf, 0, BUF_SIZE);
...
//	int blu_jitter_ret = 0;
	...
	*/
	jitter_ret = prd_rawdata_test(dev, U3_JITTER_TEST);
		...
		This will be enabled later.
	*/
        // SW49106 not use blu_jitter_test
	...
	*/
	...
	if (rawdata_ret == 0 && jitter_ret == 0)
		ret += snprintf(buf + ret, PAGE_SIZE - ret,
				"Raw Data : Pass\n");
	else
		ret += snprintf(buf + ret, PAGE_SIZE - ret,
				"Raw Data : Fail (raw:%d/jitter:%d)\n",
				rawdata_ret? 0 : 1, jitter_ret? 0 : 1);
	...
	*/
	sw49106_reset_ctrl(dev, HW_RESET_SYNC);
...
static int start_firmware(struct device *dev)
{
	...
	ret = sw49106_reg_write(dev, ADDR_CMD_REG_SIC_IMAGECTRL_TYPE, (void *)&cmd,
			sizeof(u32));
	if (ret < 0) {
		goto error;
	}
	ret = sw49106_reg_read(dev, ADDR_CMD_REG_SIC_IMAGECTRL_TYPE, &check_data,
			sizeof(u32));
	if (ret < 0) {
		goto error;
	}
...
static int stop_firmware(struct device *dev, u32 wdata)
{
	...
	/* STOP F/W to check */
	sw49106_reg_write(dev, ADDR_CMD_REG_SIC_IMAGECTRL_TYPE, &wdata, sizeof(u32));
	sw49106_reg_read(dev, ADDR_CMD_REG_SIC_IMAGECTRL_TYPE, &check_data,
			sizeof(u32));
	sw49106_reg_read(dev, ADDR_CMD_REG_SIC_IMAGECTRL_TYPE, &check_data,
			sizeof(u32));
	...
	do
	{
		...
		sw49106_reg_read(dev, ADDR_CMD_REG_SIC_GETTER_READYSTATUS,
				&read_val, sizeof(u32));
					...
					u32 cmd, u32 offset, int flag)
{
	...
	ret = sw49106_reg_write(dev, tc_tsp_test_data_offset, (u8 *)&offset, sizeof(offset));
	if (ret < 0) {
		goto out;
	}

	ret = sw49106_reg_read(dev, tc_tsp_data_access_addr, (void *)buf, size);
	if (ret < 0) {
		goto out;
	}
...
static ssize_t get_data(struct device *dev, int16_t *buf, u32 wdata)
{
	...
	case CMD_RAWDATA:
		...
		sw49106_reg_write(dev, Serial_Data_Offset,
							(u32 *)&m2_data_offset, sizeof(m2_data_offset));
		sw49106_reg_read(dev, DATA_BASE_ADDR, (int16_t *)buf, sizeof(int16_t)*ROW_SIZE*COL_SIZE);
		break;
	case CMD_BASE_DATA:
	...
	case CMD_DELTADATA:
		...
		sw49106_reg_write(dev, Serial_Data_Offset,
							(u32 *)&m2_data_offset, sizeof(m2_data_offset));
		sw49106_reg_read(dev, DATA_BASE_ADDR, (s16 *)delta_buf,
	...
	case CMD_LABELDATA:
		...
		sw49106_reg_write(dev, Serial_Data_Offset,
							(u32 *)&m2_data_offset, sizeof(m2_data_offset));
		sw49106_reg_read(dev, DATA_BASE_ADDR, (s8 *)label_buf,
	...
	case CMD_DEBUGDATA:
		...
		sw49106_reg_write(dev, Serial_Data_Offset,
							(u32 *)&m2_data_offset, sizeof(m2_data_offset));
		sw49106_reg_read(dev, DATA_BASE_ADDR, (int16_t *)buf,
...
static ssize_t show_fdata(struct device *dev, char *buf)
{
	...
	prd_read_rawdata(dev, type);
	ret = prd_print_rawdata(dev, buf, PAGE_SIZE, type);
