...
static void sw49106_get_swipe_info(struct device *dev)
{
	...
	ts->swipe[SWIPE_U].area.x1 = 0; /* 401 */;
	ts->swipe[SWIPE_U].area.y1 = 0;
	ts->swipe[SWIPE_U].area.x2 = 1079;
	ts->swipe[SWIPE_U].area.y2 = 2159;
	ts->swipe[SWIPE_U].start_area.x1 = 336;
	ts->swipe[SWIPE_U].start_area.y1 = 1914;
	ts->swipe[SWIPE_U].start_area.x2 = 744;
	ts->swipe[SWIPE_U].start_area.y2 = 2159;
	ts->swipe[SWIPE_U].wrong_dir_thres = 5;
	ts->swipe[SWIPE_U].init_ratio_chk_dist = 4;
	ts->swipe[SWIPE_U].init_ratio_thres = 100;

	ts->swipe[SWIPE_U].enable = 0;
}
...
/*
static void sw49106_qcover_active_area(struct device *dev,
		u32 x1, u32 y1, u32 x2, u32 y2)
{
	u32 active_area[4] = {x1+ACT_SENSELESS_AREA_W, y1+ACT_SENSELESS_AREA_W, \
						  x2-ACT_SENSELESS_AREA_W, y2-ACT_SENSELESS_AREA_W};
	u32 qcover_act_area[2] = {0x0, };

	TOUCH_I("%s: x1[%d], y1[%d], x2[%d], y2[%d]\n", __func__,
				active_area[0], active_area[1], active_area[2], active_area[3]);

	qcover_act_area[0] = (active_area[0]) | (active_area[1] << 16);
	qcover_act_area[1] = (active_area[2]) | (active_area[3] << 16);

	sw49106_reg_write(dev, QCOVER_ACT_AREA_START,
                &qcover_act_area[0], sizeof(u32));
	sw49106_reg_write(dev, QCOVER_ACT_AREA_END,
                &qcover_act_area[1], sizeof(u32));
}
*/
...
static int sw49106_swipe_active_area(struct device *dev)
{
	...
	active_area[0] = ts->swipe[SWIPE_U].area.x1;
	active_area[1] = ts->swipe[SWIPE_U].area.y1;
	active_area[2] = ts->swipe[SWIPE_U].area.x2;
	active_area[3] = ts->swipe[SWIPE_U].area.y2;
...
static int sw49106_swipe_control(struct device *dev, int type)
{
	...
	case SWIPE_ENABLE_CTRL:
		swipe_data = ts->swipe[SWIPE_U].enable;
	...
	case SWIPE_DIST_CTRL:
		swipe_data = ts->swipe[SWIPE_U].distance;
		ret = sw49106_reg_write(dev, SWIPE_DIST_W,
				&swipe_data, sizeof(swipe_data));
		break;
	case SWIPE_RATIO_THR_CTRL:
		swipe_data = ts->swipe[SWIPE_U].ratio_thres;
...
static int sw49106_swipe_mode(struct device *dev, u8 lcd_mode)
{
	...
	if (lcd_mode == LCD_MODE_U3) {
		...
		TOUCH_I("%s swipe[SWIPE_U].enable = %s\n", __func__,  "Disable");
	} else {
		TOUCH_I("%s swipe[SWIPE_U].enable = %s\n", __func__,  "Enable");

		swipe_data[0] = ts->swipe[SWIPE_U].enable;
		swipe_data[1] = ts->swipe[SWIPE_U].distance;
		swipe_data[2] = ts->swipe[SWIPE_U].ratio_thres;
		swipe_data[3] = ts->swipe[SWIPE_U].min_time;
		swipe_data[4] = ts->swipe[SWIPE_U].max_time;

		swipe_data[5] = ts->swipe[SWIPE_U].area.x1;
		swipe_data[6] = ts->swipe[SWIPE_U].area.y1;
		swipe_data[7] = ts->swipe[SWIPE_U].area.x2;
		swipe_data[8] = ts->swipe[SWIPE_U].area.y2;

		swipe_data[9] = ts->swipe[SWIPE_U].start_area.x1;
		swipe_data[10] = ts->swipe[SWIPE_U].start_area.y1;
		swipe_data[11] = ts->swipe[SWIPE_U].start_area.x2;
		swipe_data[12] = ts->swipe[SWIPE_U].start_area.y2;

		swipe_data[13] = ts->swipe[SWIPE_U].wrong_dir_thres;
		swipe_data[14] = ts->swipe[SWIPE_U].init_ratio_chk_dist;
		swipe_data[15] = ts->swipe[SWIPE_U].init_ratio_thres;
...
static void sw49106_debug_tci(struct device *dev)
{
	...
	sw49106_reg_read(dev, LPWG_DEBUG_FAIL_STATUS, &rdata, sizeof(rdata));
	TOUCH_I("[LPWG_DEBUG_FAIL_STATUS=[0x%x]\n", rdata);

        if (rdata & 0x1) {
            sw49106_reg_read(dev, TCI_DEBUG_FAILREASON_BUFFER, &rdata, sizeof(rdata));
            bit_mask = rdata & 0xff;
            if (bit_mask < TCI_FAIL_NUM)
                TOUCH_I("[Knock ON] TCI_DEBUG_FAILREASON_BUFFER = [0x%x]%s\n", rdata, tci_debug_str[bit_mask]);
        }
/*
        if (rdata & (0x1 << 1)) {
            sw49106_reg_read(dev, TCI_DEBUG_FAILREASON_BUFFER, &rdata, sizeof(rdata));
            bit_mask = (rdata >> 16) & 0xff;
            if (bit_mask < TCI_FAIL_NUM)
                TOUCH_I("[Knock CODE] TCI_DEBUG_FAILREASON_BUFFER = [0x%x]%s\n", rdata, tci_debug_str[bit_mask]);
        }
*/
        if (rdata & (0x1 << 2)) {
            sw49106_reg_read(dev, TCI_SWIPE_DEBUG_FAILREASON_BUFFER, &rdata, sizeof(rdata));
            bit_mask = rdata & 0xf;
            if (bit_mask < TCI_SWIPE_FAIL_NUM)
                TOUCH_I("[SWIPE] TCI_SWIPE_DEBUG_FAILREASON_BUFFER = [0x%x]%s\n", rdata, tci_swipe_debug_str[bit_mask]);
        }
	...
	if (atomic_read(&ts->state.fb) == FB_SUSPEND) {
		...
		sw49106_debug_tci(dev);
			...
			if (ts->lpwg.mode == LPWG_NONE &&
				...
				TOUCH_I("change resister setting for ESD interrupt !!!\n");
				sw49106_write_value(dev, SERIAL_SPI_EN, 0);
				sw49106_write_value(dev, SERIAL_I2C_EN, 1);
				sw49106_write_value(dev, SPI_TATTN_OPT, 0);
...
static int sw49106_swipe_enable(struct device *dev, bool enable)
{
	struct sw49106_data *d = to_sw49106_data(dev);

	TOUCH_TRACE();

	sw49106_swipe_mode(dev, d->lcd_mode);
...
static void sw49106_int_pin_work_func(struct work_struct *int_pin_work)
{
	...
	mfts_mode = touch_check_boot_mode(d->dev);

	if (mfts_mode >= TOUCH_MINIOS_MFTS_FOLDER) {
		return;
	}
...
static int sw49106_notify_etc(struct device *dev, ulong event, void *data)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw49106_data *d = to_sw49106_data(dev);
	int ret = 0;
...
static int sw49106_notify_normal(struct device *dev, ulong event, void *data)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw49106_data *d = to_sw49106_data(dev);
	int ret = 0;
...
static int sw49106_notify(struct device *dev, ulong event, void *data)
{
	...
	if (boot_mode == TOUCH_CHARGER_MODE
		...
		return sw49106_notify_etc(dev,event,data);
	}

	return sw49106_notify_normal(dev,event,data);
}

#if defined(CONFIG_DRM) && defined(CONFIG_FB)
#ifndef CONFIG_LGE_TOUCH_USE_PANEL_NOTIFY
static int sw49106_drm_notifier_callback(struct notifier_block *self,
...
static int sw49106_init_pm(struct device *dev)
{
	TOUCH_TRACE();
/* Start - kiwoo.han 2018-12-27 */
#if 0
...
/* End - kiwoo.han 2018-12-27 */
	...
	ts->drm_notif.notifier_call = sw49106_drm_notifier_callback;
#endif
#elif defined(CONFIG_FB)
	/* Start - kiwoo.han 2018-12-27 */
	struct touch_core_data *ts = to_touch_core(dev);
...
static int sw49106_upgrade(struct device *dev)
{
	...
	/* bring up */
	TOUCH_I("not support fw_upgrade\n");
	return 0;
...
static int sw49106_esd_recovery(struct device *dev)
{
	/* Start - kiwoo.han 2018-12-20 */
	//struct touch_core_data *ts = to_touch_core(dev);
	/* End - kiwoo.han 2018-12-20 */
...
//MH sw49106 code start SPI disable
...
int sw49106_check_status(struct device *dev)
{
		...
		if(ABNORMAL_IC_BIT == status){
           ...
           // Add Global reset when abnormal status
           if (status == 0 && ic_status == 0) {
                     TOUCH_I("[I2C_Recovery] %s : Perform Global Reset related to I2C\n", __func__);
                     ret = -EGLOBALRESET;
           }
...
static struct touch_driver touch_driver = {
	...
	.esd_recovery = sw49106_esd_recovery,
	.lpwg = sw49106_lpwg,
	.swipe_enable = sw49106_swipe_enable,
	.notify = sw49106_notify,
	.init_pm = sw49106_init_pm,
