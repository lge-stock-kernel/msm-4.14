...
int touch_get_dts_base(struct touch_core_data *ts)
{
	...
	/* Production Spec */
	PROPERTY_STRING_ARRAY(np, "panel_spec", ts->panel_spec, ts->def_prdcnt);
	{
		int i;

		for (i = 0; i < ts->def_prdcnt; i++) {
			TOUCH_I("panel_spec - %d:%s\n",
				i, ts->panel_spec[i]);
		}
	}
	PROPERTY_STRING_ARRAY(np, "panel_spec_mfts_folder", ts->panel_spec_mfts, ts->def_prdcnt_mfts);
	{
		int i;

		for (i = 0; i < ts->def_prdcnt_mfts; i++) {
			TOUCH_I("panel_spec_mfts - %d:%s\n",
				i, ts->panel_spec_mfts[i]);
		}
	}
