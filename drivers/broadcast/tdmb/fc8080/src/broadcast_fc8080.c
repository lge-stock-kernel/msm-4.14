...
#if defined(CONFIG_LGE_ONE_BINARY_SKU) && defined(CONFIG_MACH_SM6150_MH3)
extern int lge_get_tdmb_hwinfo(void);
#else /* defined(CONFIG_LGE_ONE_BINARY_SKU) && defined(CONFIG_MACH_SM6150_MH3) */
int lge_get_tdmb_hwinfo(void)
{
    printk("%s: non LAOP model\n", __func__);
    return false;
}
#endif /* defined(CONFIG_LGE_ONE_BINARY_SKU) && defined(CONFIG_MACH_SM6150_MH3) */
...
{
    int rc = 0;
    if(lge_get_tdmb_hwinfo() == LAOP_DTV_TDMB){
