...
static int mmc_ffu_restart(struct mmc_card *card)
{
	...
	/* Cache off CMD should not be operated in Hynix eMMC */
	if(card->cid.manfid != CID_MANFID_HYNIX){
		err = mmc_flush_cache(host->card);
		if (err) {
			pr_warn("%s: failed to flush cache(%d)!!!\n", __func__ , err);
			goto exit;
		}
	}
		...
		u8 *data, int buf_bytes)
{
	...
	if (err) {
		pr_err("FFU: %s: failed to get ext_csd(%d)\n",
				mmc_hostname(card->host), err);
		return err;
	}
	...
	if (!FFU_SUPPORTED_MODE(ext_csd[EXT_CSD_SUPPORTED_MODE])) {
		err = -EINVAL;
		pr_err("FFU: %s: FFU is not supported\n", mmc_hostname(card->host));
		goto exit;
	}

	err = mmc_host_set_ffu(card, ext_csd[EXT_CSD_FW_CONFIG]);
	if (err) {
		pr_err("FFU: %s: failed to mmc_host_set_ffu(%d)\n",
				mmc_hostname(card->host), err);
		goto exit;
	}
	...
	if (err) {
		pr_err("FFU: %s: failed to set ffu mode(%d)\n",
				mmc_hostname(card->host), err);
		goto exit;
	}
	...
	if (err) {
		pr_err("FFU: %s: failed to mmc_ffu_write(%d)\n",
				mmc_hostname(card->host), err);
		goto exit_normal;
	}
	...
	if (card->cid.manfid != CID_MANFID_HYNIX &&
		...
		if (err) {
			pr_err("FFU: %s: failed to set install mode(%d)\n",
					mmc_hostname(card->host), err);
			goto exit_normal;
		}
		...
		if (err) {
			pr_err("FFU: %s: failed to install(%d)\n",
					mmc_hostname(card->host), err);
			err = -EINVAL;
			goto exit_normal;
		}
...
exit_normal:
	...
	ret = mmc_switch(card, EXT_CSD_CMD_SET_NORMAL, EXT_CSD_MODE_CONFIG,
			MMC_FFU_MODE_NORMAL, card->ext_csd.generic_cmd6_time);
	if (ret)
		pr_err("FFU: %s: failed to unset ffu mode(%d)\n",
				mmc_hostname(card->host), err);

	if (card->cid.manfid == CID_MANFID_HYNIX ||
			!FFU_FEATURES(ext_csd[EXT_CSD_FFU_FEATURES])) {
		/* restart the eMMC */
		ret = mmc_ffu_restart(card);
		if (ret)
			pr_err("FFU: %s: failed to mmc_ffu_restart(%d)\n",
					mmc_hostname(card->host), err);
	}
