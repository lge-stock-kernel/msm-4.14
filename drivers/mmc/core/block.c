			       ...
			       struct mmc_blk_ioc_data *idata)
{
	...
	if (cmd.opcode == MMC_FFU_DOWNLOAD_OP) {
		...
		if (err) {
			pr_err("FFU: %s: failed to switch card to legacy mode: %d\n",
		...
		/* CQ enable after ffu */
		if (mmc_blk_cmdq_switch(card, md, true) != 0)
		...
		/* ffu install is excuted in mmc_ffu_download */
		return 0;
	} else if (cmd.opcode == MMC_FFU_MID_OP) {
		pr_info("FFU: %s: cid = 0x%x\n", mmc_hostname(card->host), card->cid.manfid);
		idata->buf_bytes = sizeof(card->cid.manfid);
		memcpy(idata->buf, &card->cid.manfid, idata->buf_bytes);
		return 0;
	} else if(cmd.opcode == MMC_FFU_PNM_OP) {
		pr_info("FFU: %s: pnm = 0x%x\n", mmc_hostname(card->host), card->cid.prod_name);
		idata->buf_bytes = sizeof(card->cid.prod_name);
		memcpy(idata->buf, &card->cid.prod_name, idata->buf_bytes);
		return 0;
	}
#endif
			 ...
			 int type)
{
	...
	if (host->caps & MMC_CAP_NONREMOVABLE) {
		while (err) {
			reset_count++;
			pr_err("%s:%s: failed to reset(err = %d, reset_count = %d)\n",
					mmc_hostname(host), __func__, err, reset_count);
			if (reset_count > 3) {
				pr_err("%s:%s: mmc_hw_reset() failed three times. abort it otherwise phone is hang\n",
	...
	if (host->caps & MMC_CAP_NONREMOVABLE) {
		while (err) {
			...
			pr_err("%s:%s: failed to reset(err = %d, reset_count = %d)\n",
					mmc_hostname(host), __func__, err, reset_count);
			if (reset_count > 3) {
				pr_err("%s:%s: mmc_cmdq_hw_reset() failed three times. abort it otherwise phone is hang\n",
						mmc_hostname(host), __func__);
				panic("eMMC is un-accessible");
			}
			err = mmc_cmdq_hw_reset(host);
...
#endif
{
	...
	if (ext_csd_rev >= 6) {
		...
		if (ext_csd_rev >= 7) {
			...
			/* reserved [486:309] */
			if (ext_csd_rev >= 8) {
				seq_printf(s, "[308] CMD Queuing Support, cmdq_support: 0x%02x\n", ext_csd[308]);
				seq_printf(s, "[307] CMD Queuing Depth, cmdq_depth: 0x%02x\n", ext_csd[307]);
			}
			/* reserved [306] */
		...
		seq_printf(s, "[252:249] Cache size, cache_size %d KiB\n",
				(ext_csd[249] << 0) | (ext_csd[250] << 8) | (ext_csd[251] << 16) | (ext_csd[252] << 24));
	...
	if (ext_csd_rev >= 5) {
		...
		if (ext_csd_rev >= 8)
			seq_printf(s, "[240] Cache Flushing Policy, cache_flush_policy: 0x%02x\n", ext_csd[240]);
		...
		seq_printf(s, "[234] Minimun Read Performance for 8bit at 52MHz in DDR mode, min_perf_ddr_r_8_52: 0x%02x\n", ext_csd[234]);
		/* reserved [233] */
		seq_printf(s, "[232] TRIM Multiplier, trim_mult: 0x%02x\n", ext_csd[232]);
		seq_printf(s, "[231] Secure Feature support, sec_feature_support: 0x%02x\n", ext_csd[231]);
	}
	if (ext_csd_rev == 5 || ext_csd_rev >= 7) { /* Obsolete in 4.5 */  /*---->revived in 4.51*/
	...
	if (ext_csd_rev >= 8)
		seq_printf(s, "[211] Secure Write Protect Information, secure_wp_info: 0x%02x\n", ext_csd[211]);
	...
	/* not readable [183] Bus width mode */
	/* reserved [182] */
	...
	if (ext_csd_rev >= 5) {
		...
		/* not readable [165] Start Sanitize operation */
		/* not readable [164] Manually start background operations */
	...
	/* not readable [132] Package Case Temperature is controlled */
	if (ext_csd_rev >= 6) {
		int j;
	...
	/* not readable [32] Flushing of the cache */
	if (ext_csd_rev >= 8)
		seq_printf(s, "[31] Control to turn the Barrier ON/OFF, BARRIER_CTRL: 0x%02x\n", ext_csd[31]);
	if (ext_csd_rev >= 7) {
		seq_printf(s, "[30] Mode Config, MODE_CONFIG: 0x%02x\n", ext_csd[30]);
		/* not readable [29] Mode operation codes */
		/* reserved [28:27] */
	...
	if (ext_csd_rev >= 8)
		seq_printf(s, "[15] Command Queue Mode Enable, CMDQ_MODE_EN: 0x%02x\n", ext_csd[15]);
	/* reserved [14:0] */
#endif
out_free:
